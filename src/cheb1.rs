...
pub fn chebpoints<T: RlstScalar>(kind: Kind, output: &mut [T]) {
    let n = output.len();
    let indices = (0..n).rev();
...
pub fn cheb_barycentric_weights<T: RlstScalar>(kind: Kind, output: &mut [T]) {
    let n = output.len();
    match kind {
        Kind::First => {
            assert!(n > 0);
...
mod test {
    ...
    fn test_cheb_points_first_kind() {
        ...
        chebpoints::<f64>(Kind::First, &mut values);
    ...
    fn test_cheb_points_second_kind() {
        ...
        chebpoints::<f64>(Kind::Second, &mut values);
